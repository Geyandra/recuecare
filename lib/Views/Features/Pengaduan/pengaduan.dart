...
class _PengaduanPageState extends State<PengaduanPage> {
  ...
  Future<void> getAddressFromLatLong(Position position) async {
    ...
    address = '${place.street}, ${place.subLocality}, ${place.locality}, ${place.subAdministrativeArea}, ${place.administrativeArea}';
    setState(() {});
  }

  Future<void> uploadImage(File selectedImage) async {
    final ref = FirebaseStorage.instance.ref().child('pengaduan').child(selectedImage.path.split("/").last);
    try {
                ...
                children: [
                  ...
                  CustomTextField(
                    ...
                    onTap: () async {
                      ...
                      print("Coornidate ${getPosition.latitude}, ${getPosition.longitude}");
                      getAddressFromLatLong(getPosition);
                      // print(address);
                      locationController = TextEditingController(text: address);
                      Navigator.push(context, createRoute(MapsPage(latitude: getPosition.latitude, longitude: getPosition.longitude)));
                    },
                    validator: (value) {
                      if (value == null || value.isEmpty) {
                                    ...
                                    children: [
                                      Container(
                                        ...
                                        decoration: BoxDecoration(
                                          image: DecorationImage(image: FileImage(File(listImage[index].path)), fit: BoxFit.contain),
                                        ),
                                      ),
                                      Positioned(
                                          top: 0,
                    ...
                    child: ButtonPurple(
                      ...
                      onPressed: () {
                        if (_formKey.currentState!.validate()) {
                          if (listImage.isNotEmpty) {
                            uploadImage(listImage[0]).then((value) {
                              final data = PengaduanModel(
                                ...
                                listImage.clear();
                                context.read<PengaduanProvider>().tambahPengaduan(data);
                            },);
                          }
