...
class AuthServicesProvider extends ChangeNotifier {
  ...
  void signUp(Accounts data, String password, context) async {
    try {
      await FirebaseAuth.instance.createUserWithEmailAndPassword(email: data.email, password: password).then(
  ...
  void createUserAccount(Accounts data, context) async {
    try {
      ...
      final docData = FirebaseFirestore.instance.collection("Profile").doc(user!.uid);
      data.id = user.uid;
  ...
  void getDataUserFromLogin() {
    ...
    User? user = FirebaseAuth.instance.currentUser;
    print(user!.email);
    final data = FirebaseFirestore.instance.collection("Profile").where("email", isEqualTo: user.email).snapshots().map(
          (event) => event.docs.map((e) => Accounts.fromJson(e.data())).toList(),
        );
    print("=" * 20);
    _streamUser = data;
    print(data);
  ...
  void editDataUser(Accounts data) {
    final docData = FirebaseFirestore.instance.collection("Profile").doc(data.id);
    data.id = docData.id;
    final json = data.toJson();
    docData.update(json);
    notifyListeners();
